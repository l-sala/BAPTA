    ...
    tryCatch({
      ...
      if (ncol(AP) <= sweeps) { #bypass files that have less sweeps than the required `n` (useless for the analysis) 
        ...
        for (k in 2:(ncol(AP))){
          ...
          #### Peak ####
          Peak_y <- max(AP[which(round(AP[,1],1) == Stim_art_interval_end):(nrow(AP)*0.2),k]) # this will identify the y value of the peak (max value in the first 20% of the trace after stimulation artifact)
